...
async def home(request: Request, db: Session = Depends(get_db), skip: int = 0):
    all_movies = get_all_movies(db=db, skip=skip)
...
def get_movie_data(movie_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    get_id = get_movie.id
    redirect_url = router.url_path_for("edit", movie_id=get_id)
    return RedirectResponse(redirect_url, status_code=status.HTTP_303_SEE_OTHER)
